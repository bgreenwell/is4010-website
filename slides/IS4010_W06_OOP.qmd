...
::: notes
The `GameCharacter` example earlier in this deck already carries both kinds of docstring, so point back to it rather than treating this as new syntax.

This `Book` example matches the Lab 06 stub on purpose, so students see the expected style before they open the lab. For the fuller NumPy format with Parameters and Returns sections, refer back to Week 05.

Docstrings are not checked by the tests. They are a style expectation for the rest of the course.
:::
